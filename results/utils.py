...
def write_latex_table(tbl, filename):
    ...
    with open(filename,'w') as outf:
        ...
        outf.write(tabular)
        outf.write("""\\caption{ }\n""")
        outf.write('\\label{' + filename.split('/')[-1].split('.')[0] + '}\n')
...
def output_combined_tables(da_allsys, da_nooutl, outputformat, lp_groups = None, output_dir = None):
    ...
    for name, lps in lp_groups.items(): 
        ...
        if outputformat == 'latex': 
            write_latex_table(tbl_combined, f'{output_dir}/tbl-DA-{da_allsys.correlation}-{name}-combined.tex')
        elif outputformat == 'csv': 
            tbl_combined.to_csv( f'{output_dir}/{name}-combined.csv') 
        elif outputformat == 'display_nb':
            print(name)        
            display(tbl_combined.style.applymap(fmt_nb))        
        else:
            print('available options for outputformat: csv, latex, display_nb')
        res[name] = tbl_combined
    return res

def output_tables(da_allsys, outputformat, lp_groups = None, output_dir=None):
    formatter = fmt_latex if outputformat == 'latex' else add_star
    res = {}
    if not lp_groups:
        lp_groups = get_lp_groups(da_allsys)
    ...
    for name, lps in lp_groups.items():  
        tbl_all = da_allsys.get_tables(lps,formatter)   
        if outputformat == 'latex':
            write_latex_table(tbl_all, f'{output_dir}/tbl-DA-{da_allsys.correlation}-{name}.tex') 
        elif outputformat == 'csv':
